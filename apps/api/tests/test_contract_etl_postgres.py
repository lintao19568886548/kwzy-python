"""PostgreSQL gate for the isolated synthetic contract migration drill."""
...
def _module():
    path = Path(__file__).resolve().parents[3] / "tools" / "etl" / "run_contract_etl_drill.py"
    spec = importlib.util.spec_from_file_location("run_contract_etl_drill", path)
...
def test_contract_etl_drill_reconciles_replays_and_rolls_back(tmp_path: Path) -> None:
    module = _module()
    report_path = tmp_path / "contract-etl.json"
    ...
    assert report["real_legacy_readiness"] == "BLOCKED_PENDING_AUTHORIZED_SCHEMA_AND_DESENSITIZED_SAMPLE"
    ...
    assert report["stages"]["reconciliation"]["quarantine"]["fingerprints_only"] is True
...
def test_contract_etl_refuses_unsafe_targets(url: str) -> None:
