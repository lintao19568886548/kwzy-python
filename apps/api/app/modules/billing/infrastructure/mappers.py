...
class BillMapper:
    ...
    def to_entity(m: Bill) -> BillEntity:
        return BillEntity(
            ...
            waiver_amount=Decimal(str(m.waiver_amount or 0)),
            bad_debt_amount=Decimal(str(m.bad_debt_amount or 0)),
            deferred_due_date=m.deferred_due_date,
            collection_hold=bool(m.collection_hold),
            dispute_status=m.dispute_status,
            lock_version=int(m.lock_version),
            currency=m.currency,
            source=m.source,
...
class BillLineMapper:
    ...
    def new_model(e: BillLineEntity) -> BillLine:
        return BillLine(
            tenant_id=e.tenant_id,
            bill_id=e.bill_id,
            source_schedule_id=e.source_schedule_id,
            fee_code=e.fee_code,
