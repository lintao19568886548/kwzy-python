...
| 9 | 租户服务、工单、报价、验收和评价 | User→Party→园区租户服务授权；租户自助与员工代受理；不可变派单规则草稿/发布/显式退役、确定性自动派单与人工改派；响应/解决 SLA 与幂等升级；Decimal 版本报价及租户决定；追加式人工/材料/外包/其他成本和冲正；完工证据、租户返工/验收、一次性评价；PC/移动响应式真栈、PG16 并发、真实 HTTP、合成迁移、性能与备份恢复均通过 | 员工独立移动应用、租户微信小程序、IoT/巡检来源、短信/企微/对象存储 live 和真实旧 `repair_order` 迁移由第 10/11/12/19/20 项保持缺口或阻塞；工单材料成本不冒充第 15 项库存流水 | `IMPLEMENTED_AND_VERIFIED` |
