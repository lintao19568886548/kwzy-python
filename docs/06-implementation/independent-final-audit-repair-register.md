...
> 最新精确提交验收：政策/园企服务/活动/公告 `27215ceb1bf0b39cf50a11019c861682254da086` 的 39/39 门禁、416 后端、64 浏览器、性能、备份恢复、OpenAPI/OpenSpec 与 1,200 文件密钥扫描通过。
> 原则：关键词命中必须人工分类；`CLOSED` 仅表示对应缺陷已修复并回归，不外推为全产品完成。
> repair `37d7cf7` 的独立验收、6 个 P1 与 17 个 `MISSING` 是整改起始基线；旧 Step1 complete 不具备跨业务关闭效力，后续状态变化必须有当前 HEAD 的代码与机器证据。
...
| HR-001 | P1 | HR 仅有旧 Java CRUD/轨迹证据，现仓缺员工隐私、排班、考勤、请假审批、绩效、资质和 PC 闭环 | 新增 16 张表、单一前向迁移 c5、28 个严格路径、PC 真栈、PG 并发和合成迁移；精确 `5577509` 的 380 pytest、62 Playwright、36 阶段 HTTP 与 37/37 总门禁通过 | CLOSED_LOCAL_PRODUCT_SCOPE |
| SEC-009 | P1 | HR 原始手机号/证件/证书号、精确坐标/轨迹、伪造权限、跨租户/园区和小写审批定义码均需 fail closed | 仅存掩码+独立密钥指纹，坐标只参与请求校验不持久化；数据库派生权限/404 隔离/参数污染/幂等冲突回归；审批码规范化并由真实 HTTP 回归覆盖 | CLOSED |
| MIG-LOCAL-004 | P1 | HR 旧数据缺可重复执行、中断恢复、隔离、隐私对账和回滚演练 | 合成 ETL 完成 dry/interruption/apply/reapply/reconcile/rollback；2 员工/1 班次版本/2 排班/3 打卡/2 汇总/1 请假/3 隔离，不伪造绩效、资质或工资 | CLOSED_SYNTHETIC_SCOPE |
| ENGAGEMENT-001 | P1 | 现仓缺政策治理、园企服务、园区活动、公告送达与员工/租户 PC 真栈 | 新增单一前向 e7、20 张表、47 个严格路由、原生审批/幂等/并发/冻结受众/站内送达和 PC 真栈；精确 `27215ce` 的 416 pytest、64 Playwright 与 39/39 总门禁通过 | CLOSED_LOCAL_PRODUCT_SCOPE |
| MIG-LOCAL-005 | P1 | 园企聚合缺中断、断点、幂等、隔离、对账与回滚演练 | 合成 ETL 8 阶段通过；政策/服务单/活动容量/报名/公告 audience/delivery/read 精确对账，5 条不可信记录隔离，不伪造官方、提供商或外部送达 | CLOSED_SYNTHETIC_SCOPE |
...
| CAP-003 | 其余组合业务闭环缺失 | 园企纵切关闭后，能力矩阵仍有 5 项为 MISSING | 按矩阵逐项实现，或取得有依据/责任人的退休或延期批准 | 产品/领域负责人（待指定）；MISSING |
