...
> 最新精确提交闭环：HR、排班、考勤、绩效与资质；`5577509a6ca2a473107038be1316a0ea9252fc8d` 的 37/37 全量门禁通过
> 上一 clean-SHA：档案、电子签章与印章 `b04d738bc75e865e1c6bfb180e93dc9b95109cf3`（36/36，已正常推送）
...
HR/排班/考勤/绩效/资质纵切已补齐为本地产品闭环：手机号、证件号和证书号只存掩码与独立密钥指纹；员工与系统身份显式绑定；不可变班次版本、日期排班、在职/请假/重复排班门禁；考勤策略与只保存配置引用的地点、幂等打卡、缺卡异常和有因调整；请假使用平台原生 Approval；绩效禁止自评并支持发布/确认；资质必须附证据并支持复核、吊销和到期待办。工资核算、真实考勤设备/定位/生物识别和无权威来源的旧绩效/资质历史均未伪造。
...
下述档案/签章/印章段落为上一纵切 clean-SHA 证据，继续保留：
...
最新能力矩阵为 12 项 `IMPLEMENTED_AND_VERIFIED`、1 项 `BLOCKED`、7 项 `MISSING`。因此本报告仍是 `BLOCKED`；HR 组合能力的本地 PC 产品闭环关闭，绝不外推为工资核算、真实考勤设备、员工独立移动端、小程序、全业务、真实旧数据迁移或生产完成。

### 当前交付基线

| 项 | 当前独立核验结果 |
| --- | --- |
| 分支 / 精确 HEAD | `feat/full-rebuild-completion` / `5577509a6ca2a473107038be1316a0ea9252fc8d` |
| 实现与修复提交 | `370c7f8` HR 本地产品闭环；`5577509` HR 真实 HTTP 与总门禁扩展 |
| 远程同步 | HR 提交本地领先，待证据/规格收尾后正常、非 force 推送；本轮未触达 main |
| 工作树保护 | 全量 Playwright 重生成的其他纵切 27 张截图属于既有用户修改，保持未暂存且不删除/不还原；本轮只提交 HR 证据、总控文档和 OpenSpec 收尾 |
| 运行入口 | API `uvicorn app.main:app`；PC `npm run dev` / `npm run build`；员工移动端和租户小程序无目录/启动方式 |
| 环境与敏感文件 | `.env.example` 三处均为非生产占位；1,091 个 tracked/untracked non-ignored 文件扫描 0 hits；未自动连接生产 |
...
环境样例为 `apps/api/.env.example`、`infra/postgres-test/.env.example`、`infra/production/production.env.example`。最新精确提交复验的敏感文件名检查与 1,091 文件内容扫描通过；未发现被跟踪的真实密钥、Token、密码、数据库文件或 PII。样例值均为明确的非生产占位。
...
```text
j6e24f9a1c08 -> k7f35a0b2d19 -> l8a46b1c3e20 -> m9b57c2d4e31 -> n0c68d3e5f42 -> o1d79e4f6a53 -> p2e80a5b7c64 -> q3f91b6c8d75 -> r4a02c7d9e86 -> s5b13d8e0f97 -> t6c24e9f1a08 -> u7d35f0a2b19 -> v8e46a1b3c20 -> w9f57b2c4d31 -> x0a68c3d5e42 -> y1b79d4e6f53 -> z2c80e5f6a64 -> a3d91f6a7b75 -> b4ea2c7d8f86 -> c5f02d8e9c87 (head)
```
...
- 空库 base → head 通过，`current == heads == c5f02d8e9c87`；c5 down 到 b4 再 up 通过，专项 `alembic check` 无待生成迁移。
- 备份、删除测试恢复库、重建和恢复复核当前 head 与 162 张表；核心表、索引、外键、唯一性和 boolean 契约由 metadata/PG 测试覆盖。
...
当前 HR 精确 SHA `5577509` 全量验收为 37/37 步、380 pytest、62 条 Playwright、3 文件/6 Vitest、150 modules production build、16 条 OpenAPI 契约及 YAML strict、OpenSpec strict 111/111、1,091 文件 secrets scan 0 hits，耗时 656,601 ms。HR 36/36 真实 HTTP、PG 并发、隐私/幂等/隔离回归通过。下表保留初始 repair 精确 SHA `a9267d4` 的历史基线，不能覆盖最新数字。
...
当前 HR 精确 `5577509` 全量验收使用 `pg_dump -Fc` 生成 1,697,402 bytes dump；删除并重建临时恢复库后 `pg_restore`，恢复 162 张表并再次删除恢复库。HR 合成迁移为 2 员工/1 班次版本/2 排班/3 打卡/2 汇总/1 请假/3 隔离，原始 PII/精确坐标列与伪造绩效、资质、工资结果均为 0。临时 dump 按安全策略未提交 Git。
...
- 0 failures，错误率 0.0%，145.596 req/s。
- p50 161.502 ms，p95 264.836 ms，p99 324.036 ms，max 372.069 ms。
- 门槛 p95 ≤ 500 ms、错误率 ≤ 1%、吞吐 ≥ 20 req/s，结果 PASS。
- HR 专项真实 HTTP 为 36/36 阶段、1,261.16 ms，验证 PII/坐标最小化、原生审批、小写定义码、幂等冲突、请假排班门禁、绩效/资质和跨租户隔离；全量精确提交报告保存在 `evidence/workforce-scheduling-attendance-performance/acceptance-clean-5577509.json`。
...
| API/OpenAPI 漂移 | 已修复，16/16 契约、workforce 28 个路径的 runtime/YAML 方法精确一致且 YAML strict 通过 |
...
| 密钥/Token/DB/PII | 精确提交复验 tracked + untracked non-ignored 1,091 文件扫描通过、0 hits；既有 clean-SHA 证据继续有效；HR 手机/证件/资质号只保存掩码与独立密钥指纹，精确坐标/轨迹不持久化，环境样例为非生产占位 |
...
PC 真实浏览器验证使用 PostgreSQL、FastAPI、生产 Vite 构建和真实 HTTP；当前全量 62 条 Playwright 无 skip。HR 截图人工复核覆盖桌面资质、平板考勤异常与 390px 离线重试；既有合同/组织/审批/工作台/资产/Party/应收/工单/设施/档案证据继续保留。页面无假数据、乱码、遮挡或移动端横向溢出；证据位于 `evidence/workforce-scheduling-attendance-performance/` 等受信目录。
...
旧 Java 证据仓静态规模为 54 Controllers、约 484 HTTP mappings、316 Vue 文件、30 SQL DDL 表。当前 Python 仓库缺少两端且仍有 7 个组合能力为 `MISSING`、1 项迁移为 `BLOCKED`，因此旧系统替代为 `BLOCKED`。
...
| OA/ERP/采购/协作平台与真实 HR 设备 | HR 本地业务已闭环；其余适配器缺失 | HR LOCAL_PRODUCT_ONLY；外部 NOT_CONNECTED/MISSING |
...
| --- | --- | --- |
| P0 | 0 | 当前证据未发现 P0 |
| P1 | 6 | 员工移动端；租户小程序；7 项能力矩阵缺失的产品闭环；真实旧数据迁移；外部适配器完整性/真实联调；远程预发监控容量灾备 |
...
能力矩阵汇总为 12 项 `IMPLEMENTED_AND_VERIFIED`、1 项 `BLOCKED`、7 项 `MISSING`。完整细节见 `full-rebuild-traceability-matrix.md`。因为 P1 不为 0、两端缺失、真实迁移和旧系统替代未完成，完成分支不得合并 main。
