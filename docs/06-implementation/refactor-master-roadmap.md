...
> 当前独立验收基线：设施设备/周巡检/IoT 闭环精确提交 `e28678b5785b7f8bf03141bd7cffbd162932315b` 的 35/35 后端/前端/浏览器/性能/迁移门禁通过并正常推送；租户服务/工单 `ffa72e2531396997cfe24ef1b1e42526cd4735fe` 证据继续有效
...
| 服务与运营业务 | Party 绑定租户服务、员工/租户受理、规则派单/改派、SLA、报价、追加式成本、完工证据、返工/验收/评价；设备台账/历史、模板/周计划、类型化巡检、异常/漏检转工单、IoT 绑定/告警及 PC 真栈 | 员工移动端；库存/供应商联动；HR、供应链、园企服务；真实 IoT live 联调 | TENANT_SERVICE_AND_FACILITY_CLOSED_LOCAL_PRODUCT_SCOPE / REMAINDER_MISSING |
...
9. `2026-08-15-complete-facility-device-inspection-iot`：实现 `00aaab6`，权限/浏览器竞态及证据稳定性修复至 `e28678b`，均正常推送；唯一 head `z2c80e5f6a64`，精确提交 352 后端、60 浏览器、32 个 runtime/YAML 方法、1000/25 p95 320.076 ms/135.6 RPS/0 错误、1,495,479 bytes/130 表备份恢复、设施合成 ETL 与 982 文件密钥扫描通过；本地产品纵切关闭，真实 IoT/旧数据仍保持 NOT_CONNECTED/BLOCKED。
10. 下一业务纵切按证据顺序关闭档案/签章/印章，再处理 HR、供应链、园企服务、驾驶舱、AI/集成。
11. 建设员工移动端与租户微信小程序并完成真实 API/数据库/浏览器（或小程序运行器）旅程，不能用 WebView 壳或本地 JSON 替代。
12. 数据负责人/DBA 提供经授权旧 schema dump 与脱敏快照；在隔离 PG16 重跑全量/增量/中断/回滚/对账。
13. 集成负责人提供沙箱协议和短期凭据；SRE 在获批预发完成容量、监控告警和灾备。只有能力矩阵无 `MISSING/BLOCKED`、三端关键旅程通过、P0/P1=0 且生产部署另获人工授权后，才能非强制合并 main 并在最终 main 重验。
