...
> 当前独立验收基线：政策/园企服务/活动/公告闭环精确提交 `27215ceb1bf0b39cf50a11019c861682254da086` 的 39/39 后端/前端/浏览器/性能/迁移门禁通过；既有供应链、HR、档案/签章/印章和设施设备/周巡检/IoT 证据继续有效
...
| 服务与运营业务 | Party 绑定租户服务、完整工单；设备/周巡检/IoT；员工档案/排班/考勤/绩效/资质；供应链；版本化政策/园企服务/活动/公告与员工/租户 PC 真栈 | 员工移动端；真实政府 feed、服务商、外部通知、ERP/WMS/供应商门户、IoT/考勤设备 live 联调 | LOCAL_PRODUCT_SCOPES_CLOSED / MOBILE_AND_EXTERNAL_REMAINDER |
...
12. `2026-08-20-complete-supply-procurement-inventory-outsourcing`：实现 `3e3c815` 已正常推送；唯一 head `d6a13e9f0b98`，精确提交 38/38、390 后端、63 浏览器、41 个供应链 runtime/YAML 方法、56 阶段真实 HTTP、1000/25 p95 297.794 ms/139.457 RPS/0 错误、1,830,705 bytes/182 表备份恢复和供应链合成 ETL 通过；40/40 任务、10 份主规格同步、归档后 strict 129/129，本地产品纵切关闭，真实 ERP/WMS/供应商门户和旧数据仍 NOT_CONNECTED/BLOCKED。
13. `2026-08-20-complete-park-enterprise-policy-service-engagement`：实现 `27215ce` 已正常推送；唯一 head `e7b24f0a1c09`，精确提交 39/39、416 后端、64 浏览器、47 个 engagement 路由、1000/25 p95 316.95 ms/118.28 RPS/0 错误、1,976,440 bytes/202 表备份恢复和 engagement 合成 ETL 通过；41/41 任务、7 份主规格同步、归档后 strict 136/136；本地产品纵切关闭，政府 feed/外部服务商/通知及真实旧数据仍 NOT_CONNECTED/BLOCKED。
14. 下一业务纵切按证据顺序处理驾驶舱、指标下钻、多园区比较和 AI/人工确认门禁。
15. 建设员工移动端与租户微信小程序并完成真实 API/数据库/浏览器（或小程序运行器）旅程，不能用 WebView 壳或本地 JSON 替代。
16. 数据负责人/DBA 提供经授权旧 schema dump 与脱敏快照；在隔离 PG16 重跑全量/增量/中断/回滚/对账。
17. 集成负责人提供沙箱协议和短期凭据；SRE 在获批预发完成容量、监控告警和灾备。只有能力矩阵无 `MISSING/BLOCKED`、三端关键旅程通过、P0/P1=0 且生产部署另获人工授权后，才能非强制合并 main 并在最终 main 重验。
