...
| 阶段 | 已关闭范围 | 仍需关闭 | 状态 |
| --- | --- | --- | --- |
| 平台安全与工程底座 | 会话/JWT/密码、scope、集团/区域/园区历史归属、岗位任职、服务端字段策略、附件、生产配置、PG pool/readiness、CI/容器/Runbook | 完整审计/审批、远程监控/告警/灾备证据 | IN_PROGRESS |
...
1. `complete-platform-organization-governance`：集团→区域→园区历史归属、岗位任职、字段策略、PC、PG16、HTTP/浏览器和合成 ETL 已通过；OpenSpec 27/27，提交 `8bb8770` 已正常推送并在 clean SHA 上再次通过 25/25 总闸门，六份主规格已同步并归档。
2. 下一平台纵切：完整审批中心、审计中心，以及消息/规则/定时任务驱动的多角色工作台。
3. 依次关闭资产模板、招商自动分配/意向审批、企业画像、账收匹配催缴、租户服务、设备巡检 IoT、档案签章、HR、供应链、园企服务、驾驶舱、AI/集成。
4. 建设员工移动端与租户微信小程序并完成真实 API/数据库/浏览器（或小程序运行器）旅程，不能用 WebView 壳或本地 JSON替代。
5. 数据负责人/DBA 提供经授权旧 schema dump 与脱敏快照；在隔离 PG16 重跑全量/增量/中断/回滚/对账。
6. 集成负责人提供沙箱协议和短期凭据；SRE 在获批预发完成容量、监控告警和灾备。只有能力矩阵无 `MISSING/BLOCKED`、三端关键旅程通过、P0/P1=0 且生产部署另获人工授权后，才能非强制合并 main 并在最终 main 重验。
