...
> 当前独立验收基线：供应商/采购/库存/领用/外包闭环精确提交 `3e3c815285025e0462122cbc845c447995c28aaf` 的 38/38 后端/前端/浏览器/性能/迁移门禁通过；既有 HR、档案/签章/印章和设施设备/周巡检/IoT 证据继续有效
...
| 服务与运营业务 | Party 绑定租户服务、完整工单；设备/周巡检/IoT 本地闭环；员工隐私档案、排班/考勤、原生审批请假、绩效/资质；供应商治理、采购、库存领退盘点与外包验收及 PC 真栈 | 员工移动端；政策/园企服务/活动/公告；真实 ERP/WMS/供应商门户、IoT/考勤设备 live 联调 | TENANT_SERVICE_FACILITY_HR_AND_SUPPLY_CLOSED_LOCAL_PRODUCT_SCOPE / REMAINDER_MISSING |
...
13. 下一业务纵切按证据顺序关闭政策、园企服务、活动和公告，再处理驾驶舱、AI/集成。
14. 建设员工移动端与租户微信小程序并完成真实 API/数据库/浏览器（或小程序运行器）旅程，不能用 WebView 壳或本地 JSON 替代。
15. 数据负责人/DBA 提供经授权旧 schema dump 与脱敏快照；在隔离 PG16 重跑全量/增量/中断/回滚/对账。
16. 集成负责人提供沙箱协议和短期凭据；SRE 在获批预发完成容量、监控告警和灾备。只有能力矩阵无 `MISSING/BLOCKED`、三端关键旅程通过、P0/P1=0 且生产部署另获人工授权后，才能非强制合并 main 并在最终 main 重验。
